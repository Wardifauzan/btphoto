jQuery(document).ready(function ($) {
    var err_cont = $('<div class="sdm_sf_tpl_2_error_container"></div>');

    $('input.sdm_sf_tpl_2_input_name,input.sdm_sf_tpl_2_input_email').keypress(function () {
	$(this).siblings('.sdm_sf_tpl_2_error_container').remove();
    });

    // SDM squeeze form download button
    $('.sdm_sf_tpl_2_input_submit').click(function (e) {

	// Get name, email and validate it
	var curr_form_div = $(this).closest('.sdm_sf_tpl_container');
	var name_input = curr_form_div.find('.sdm_sf_tpl_2_input_name');
	var email_input = curr_form_div.find('.sdm_sf_tpl_2_input_email');
	...
	// If name not present
        ...
        if (name_required == 'yes'){
            if (get_name === '') {
                if (name_input.siblings('.sdm_sf_tpl_2_error_container').length === 0) {
                ...
                if (name_input.siblings('.sdm_sf_tpl_2_error_container').length !== 0) {
                    name_input.siblings('.sdm_sf_tpl_2_error_container').remove();
                }
            }
        }
	...
	if (get_email === '') {
	    if (email_input.siblings('.sdm_sf_tpl_2_error_container').length === 0) {
	    ...
	    if (email_input.siblings('.sdm_sf_tpl_2_error_container').length !== 0) {
		email_input.siblings('.sdm_sf_tpl_2_error_container').remove();
	    }
	}
