...
function sdm_sf_render_fancy1_form($a) {
...
// Get SDM download item info
    ...
    $email_delivary_msg_handler = sdm_sf_handle_email_delivary_msg( $a['id'], 1 );
    //The squeeze form
    $content = '';
    $content .= '<div class="sdm_sf_tpl_container sdm_sf_tpl_1_container">';

    $content .= '<div class="sdm_sf_tpl_1_feature_image">';
    $content .= '<img src="' . $post_image . '" />';
    $content .= '</div>';

    $content .= '<div class="sdm_sf_tpl_1_title">';
    $content .= '<h3>' . $post_title . '</h3>';
    $content .= '</div>';

    $content .= '<div class="sdm_sf_tpl_1_description">';
    ...
    $content .= '<div class="sdm_sf_tpl_1_form">';
    if ($dl_delivered_by_email) {
        ...
        $content .= '</div>'; //end sdm_sf_tpl_1_email
        // Check if reCaptcha is enabled.
        if ($reCaptcha){
            $content .= '<div class="sdm-sf-recaptcha-box sdm-sf-tpl1-recaptcha-box">';
    ...
    $content .= '</div>'; //end sdm_sf_tpl_1_form

    $content .= '</div>'; //end sdm_sf_tpl_1_container
    return $content;
}
